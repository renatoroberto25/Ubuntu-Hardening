...
configure_docker_rootless() {
    ...
    if [ "$DISABLE_ROOTFUL_DOCKER" = "true" ]; then
        ...
        manual_step "Docker rootful" "DISABLE_ROOTFUL_DOCKER=false. Para fechar este item: garantir que devs nao estejam no grupo docker com getent group docker e gpasswd -d <usuario> docker, ou reexecutar com DISABLE_ROOTFUL_DOCKER=true."
    fi
...
"
    ...
    manual_step "Egress firewall" "Sem UFW egress local. Para fechar este item: confirmar regra de saida no firewall/proxy corporativo ou implementar politica local aprovada."
}
